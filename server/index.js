...
// An operator may only act on their own station; officer/admin can act on any.
function canAccessStation(user, stationId) {
  if (user.role === 'officer' || user.role === 'admin') return true;
  if (user.role === 'operator') return user.stationId === stationId;
  return false;
}
...
// ---------------------------------------------------------------------------
// Comms / resilience -- satellite + HF radio failover, priority buffer
// ---------------------------------------------------------------------------
app.get('/api/comms/:stationId/status', authMiddleware, (req, res) => {
  const { stationId } = req.params;
  if (!resilience.STATIONS.includes(stationId)) return res.status(404).json({ error: 'Unknown station' });
  res.json(resilience.getCommsStatus(stationId));
});

app.post('/api/comms/:stationId/toggle-satellite', authMiddleware, (req, res) => {
  const { stationId } = req.params;
  if (!resilience.STATIONS.includes(stationId)) return res.status(404).json({ error: 'Unknown station' });
  if (!canAccessStation(req.user, stationId)) {
    return res.status(403).json({ error: 'Not authorized for this station' });
  }
  const current = resilience.getCommsStatus(stationId);
  const newState = resilience.setSatellite(stationId, !current.satelliteUp);
  res.json({ satelliteUp: newState.satelliteUp, hfRadioActive: newState.hfRadioActive });
});
...
app.listen(PORT, () => {
  ...
  resilience.start();
  console.log('Resilience simulation started for:', resilience.STATIONS.join(', '));
