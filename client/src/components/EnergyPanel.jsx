...
export default function EnergyPanel({ stationId }) {
  ...
  useEffect(() => {
    ...
    const load = (showSpinner) => {
      ...
      setError(null);
      return Promise.all([getEnergyLatest(stationId), getEnergyHistory(stationId)])
    ...
    load(true);
    // Poll for updates -- the change-detection engine on the backend means
    // most ticks won't move these numbers, but real ones will show up here.
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {latest.generators.map((g) => (
          <div key={g.id} className="bg-panel border border-border rounded-xl p-4">
            ...
            <div className="text-xs text-muted mt-1">{g.tempC}&deg;C</div>
          </div>
        ))}
      </div>
    </div>
