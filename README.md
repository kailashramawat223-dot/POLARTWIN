...
# 🛰️ Resilience: Change-Detection, Local Alerts & Multi-Path Comms

Real Antarctic stations don't have a reliable, cheap, always-on internet
link back to India. PolarTwin's resilience layer simulates the parts of
the pipeline that deal with that reality, per station, independently.

### 1. Change-detection engine

Instead of streaming every sensor tick, each station's simulated edge
gateway only transmits a reading when it **crosses a threshold** (a
generator's load moves meaningfully, the temperature shifts, etc.) or
when a **periodic heartbeat** is due. This is why the Energy and
Environment charts don't move on every single tick &mdash; most ticks are
genuinely unchanged and are suppressed before they'd ever hit a real
satellite link.

### 2. Local alert system

Safety-relevant thresholds (generator overload/overheat, extreme wind,
out-of-range temperature) are evaluated **locally, every tick**, against
the station's raw sensor state &mdash; regardless of whether that tick's
routine reading made it into the shared store. An alert firing does not
wait for, or depend on, the satellite link.

### 3. Satellite + HF radio failover

Each station has its own independent comms state: a primary satellite
uplink and an HF radio backup. When the satellite is down, only
high-priority traffic (alerts) rides the HF radio; routine telemetry
queues locally instead.

### 4. Priority data buffer

Routine telemetry that couldn't go out while the satellite was down is
held in a local buffer, in order, and flushed into the shared store the
moment the satellite comes back &mdash; exactly as if it had just arrived.

``` mermaid
flowchart TD
    A["Simulated Sensors"] --> B["Change-Detection Engine"]
    B -- "no meaningful change" --> Z["Suppressed \u2014 nothing sent"]
    B -- "threshold change / heartbeat" --> C{"Satellite Up?"}
    B -- "safety threshold breached" --> L["Local Alert System"]

    L --> M{"Satellite Up?"}
    M -- Yes --> SAT1["\ud83d\udef0\ufe0f Satellite"]
    M -- No --> HF["\ud83d\udcfb HF Radio Backup"]

    C -- Yes --> SAT2["\ud83d\udef0\ufe0f Satellite"]
    C -- No --> BUF["\ud83d\udcbe Local Priority Buffer"]
    BUF -- "satellite restored" --> SAT2

    SAT1 --> NCPOR["NCPOR Dashboard"]
    SAT2 --> NCPOR
    HF --> NCPOR
```

This is exposed in the UI as a **Resilience** tab on every station page
(satellite/HF radio/buffer status, a demo "Simulate satellite failure"
toggle, and a recent comms-events log), a compact status strip on the
Overview tab, a **Network resilience** summary on the Officer Dashboard
covering both stations at once, and channel badges (`via satellite`,
`via HF radio`) on alerts so it's visible exactly how each one got
through.

> **Prototype note:** the satellite/HF radio link itself is a simulated
> toggle, not real modem/radio hardware, and the two stations share one
> in-memory data store rather than a true station-side vs. NCPOR-side
> split. The change-detection, local-alerting, failover, and buffering
> *logic* is real and runs continuously; only the physical comms
> hardware is mocked.
...
## Main API Groups
  ...
  Comms Status            `GET /api/comms/:stationId/status`         Authenticated

  Toggle Satellite        `POST /api/comms/:stationId/toggle-satellite` Own station Operator, or Officer/Admin
...
### What the prototype now simulates

The resilience layer added to this prototype (see [🛰️ Resilience](#%EF%B8%8F-resilience-change-detection-local-alerts--multi-path-comms)
above) already models the shape of the left half of this diagram in
software:

-   ✅ Edge gateway behaviour (change-detection engine, per station)
-   ✅ Satellite-down failover to a backup channel (HF radio) and a local
    priority buffer
-   ✅ A local alert engine that doesn't depend on the link being up

What's still simulated rather than real:

-   Real IoT sensor integration (currently a numerical simulation)
-   Actual satellite/HF radio hardware (currently a UI toggle)
-   A true station-side vs. NCPOR-side data split (currently one shared
    store)

### Planned production upgrades

-   Real IoT sensor integration
-   Real satellite modem / HF radio hardware integration
-   Redundant edge gateway units with automatic failover between them
-   Power management for the Antarctic polar night (solar + wind +
    battery bank, with load-prioritized shedding)
...
The resilience layer (change-detection engine, local alerts, satellite/HF
radio failover, priority buffer) runs real logic continuously in the
backend, but against simulated sensor readings and a simulated comms
link &mdash; not physical hardware.
...
# 📌 Project Status
  ...
  Live Sensor Simulation   ✅ Implemented
  Change-Detection Engine  ✅ Implemented
  Local Alert System       ✅ Implemented
  Satellite / HF Failover  ✅ Simulated
  Priority Data Buffer     ✅ Implemented
  Real IoT Sensors         🔜 Planned
  Real Satellite/HF Radio  🔜 Planned
  Redundant Edge Gateway   🔜 Planned
  Polar-Night Power Mgmt   🔜 Planned
